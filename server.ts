...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'dist/ft-theater/browser');
  ...
  const template = fs
    .readFileSync(
      path.join(
        join(process.cwd(), 'dist/ft-theater/browser'),
        'index.html'
      )
    )
    .toString();
    ...
    res.render(
      ...
      async (err, html) => {
        ...
        if (
          ...
          const talent = {
            name: `Fuck Theater Research ${group?.PageTitle}`,
